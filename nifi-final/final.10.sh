...
########################################
# Inject parameters into flow JSON
########################################

inject_parameters() {

 log_section "Injecting parameters into flow JSON"

 build_param_json
 echo "$PARAM_JSON" | jq .

 TMP_FLOW="/tmp/flow_injected.json"

 jq --argjson params "$PARAM_JSON" '
 (.. | objects | select(has("parameters")) | .parameters) |=
 map(
   if .parameter.name as $n | $params[$n] then
     .parameter.value = $params[$n]
   else .
   end
 )
 ' "$FLOW_FILE" > "$TMP_FLOW"
...
########################################
# Update comments for existing PG
########################################
...
update_pg_comments() {
 ...
 log_info "Updated process group comments"
}

########################################
# Update ALL parameter contexts
########################################

update_parameter_contexts() {

 log_section "Updating all parameter contexts"
 ...
 echo "$CONTEXTS" | jq -c '.parameterContexts[]' | while read -r ctx; do
   ...
   log_info "Processing context: $PC_NAME"
   ...
   HAS_PARAMS=$(echo "$CURRENT" | jq '.component.parameters | length')

   if [ "$HAS_PARAMS" -eq 0 ]; then
     log_warn "No parameters in $PC_NAME, skipping"
     continue
   fi
   ...
   log_info "Updated context: $PC_NAME"

 done

 log_info "All parameter contexts processed"
}
